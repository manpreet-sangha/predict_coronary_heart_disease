...
def render(df: pd.DataFrame) -> None:
    ...
    st.markdown(
        "Two complementary criteria are used to rank features: **Mutual Information (MI)** "
        "captures non-linear dependencies between each feature and the CHD target, while "
        "the **ANOVA F-test** measures linear separability between classes. Using both guards "
        "against misleading rankings from either criterion alone. Scores are normalised to "
        "[0, 1] so they can be compared side by side. The orange line shows the mean score "
        "across both criteria — a robust overall importance ranking."
    )
    ...
    # ── Chi-square + full table in expander ────────────────────────────────
    with st.expander("Chi-Square Test & Full Scores Table"):
