...
Eleven features were used: the nine original features plus two derived
interaction terms, \texttt{age\_tobacco} ($\text{age}\times\text{tobacco}$,
a lifetime smoking burden proxy) and \texttt{age\_famhist}
($\text{age}\times\text{famhist}$, genetic risk compounding with age).
Preprocessing: \texttt{famhist} encoded as 0/1; skewed features
(\texttt{sbp}, \texttt{tobacco}, \texttt{ldl}, \texttt{alcohol})
log\,\textsubscript{1p}-transformed; all features standardised with
