...
(Table~\ref{tab:cv}). Decision Tree performed worst (CV-AUC\,=\,0.557),
confirming that a single tree overfits without pruning. Random Forest
achieved the highest CV-AUC (0.715), marginally ahead of SVM (0.712) and
Gradient Boosting (0.709). Random Forest was therefore selected for
...
\midrule
Decision Tree     & 0.557  & 0.422 & 0.596 \\
KNN               & 0.659  & 0.464 & 0.675 \\
Gradient Boosting & 0.709  & 0.481 & 0.683 \\
SVM (RBF)         & 0.712  & 0.508 & 0.721 \\
\textbf{Random Forest} & \textbf{0.715} & \textbf{0.489} & \textbf{0.686} \\
\bottomrule
\end{tabular}}
\end{minipage}
...
\texttt{n\_estimators=500} (tuned CV-AUC\,=\,0.748). Shallow trees
...
AUC\,=\,0.783, F1\,=\,0.597, and accuracy\,=\,0.753
(Table~\ref{tab:final}).
Ridge logistic regression attains a higher AUC (0.810) suggesting better
overall discrimination, while Random Forest surpasses it on F1 (0.597
vs.\ 0.519) and accuracy (0.753 vs.\ 0.720) by improving CHD recall from
0.44 to 0.53. Feature importances confirm that \texttt{age} (0.275),
\texttt{tobacco} (0.165), and \texttt{ldl} (0.150) are the dominant
predictors, fully consistent with EDA rankings and ridge coefficients
\cite{rehman2025predicting, ogunpola2024machine}.
...
Model & AUC-ROC & F1 & Precision & Recall \\
\midrule
Ridge LR ($C=0.01$)          & \textbf{0.810} & 0.519 & 0.636 & 0.438 \\
Random Forest (tuned) & 0.783 & \textbf{0.597} & \textbf{0.680} & \textbf{0.531} \\
\bottomrule
\end{tabular}}
\end{table}
