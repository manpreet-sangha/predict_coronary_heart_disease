"""
oc_extra_trees.py — Extra Trees Classifier for CHD Prediction
==============================================================
WHY INCLUDED:
    Extra Trees (Extremely Randomised Trees) extends Random Forest by
    also randomising the split thresholds, not just the feature subsets.
    This additional randomisation further reduces variance at the cost of
    slightly higher bias, and can outperform standard Random Forest on
...
def tune_and_evaluate(
    ...
    pd.DataFrame(
        classification_report(y_test, y_pred,
                              ...
                              output_dict=True)
    ).T.to_csv(os.path.join(output_dir, "et_classification_report.csv"))
    ...
    fi_df.to_csv(os.path.join(output_dir, "et_feature_importance.csv"), index=False)
...
def _save_figures(model, y_test, y_pred, y_prob, auc, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_et_roc_curve.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_et_confusion_matrix.png"), dpi=150)
    plt.close()
