"""
oc_decision_tree.py — Decision Tree Classifier for CHD Prediction
==================================================================
WHY INCLUDED:
    Decision Trees are the interpretable baseline for non-linear classifiers.
    A single unpruned tree tends to overfit on small datasets; max_depth and
    min_samples_leaf are tuned via GridSearchCV to control complexity.
...
def tune_and_evaluate(
    ...
    pd.DataFrame(
        classification_report(y_test, y_pred,
                              ...
                              output_dict=True)
    ).T.to_csv(os.path.join(output_dir, "dt_classification_report.csv"))
...
def _save_figures(model, y_test, y_pred, y_prob, auc, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_dt_roc_curve.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_dt_confusion_matrix.png"), dpi=150)
    plt.close()
