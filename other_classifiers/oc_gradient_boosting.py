"""
oc_gradient_boosting.py — Gradient Boosting Classifier for CHD Prediction
==========================================================================
WHY INCLUDED:
    Gradient Boosting builds an ensemble sequentially: each tree corrects the
    residual errors of the previous one (boosting). It is generally the
    strongest off-the-shelf algorithm for tabular binary classification.
    The learning rate and tree depth are jointly tuned to balance bias and
    variance on this small dataset (462 observations).
...
def tune_and_evaluate(
    ...
    pd.DataFrame(
        classification_report(y_test, y_pred,
                              ...
                              output_dict=True)
    ).T.to_csv(os.path.join(output_dir, "gb_classification_report.csv"))
...
def _save_figures(model, y_test, y_pred, y_prob, auc, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_gb_roc_curve.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_gb_confusion_matrix.png"), dpi=150)
    plt.close()
    ...
    fi_df.to_csv(os.path.join(output_dir, "gb_feature_importance.csv"), index=False)

    fig, ax = plt.subplots(figsize=(6, 4))
    ax.barh(fi_df["Feature"], fi_df["Importance"], color="steelblue")
    ax.set_xlabel("Feature Importance (mean decrease in impurity)")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_gb_feature_importance.png"), dpi=150)
    plt.close()
