"""
oc_lda.py — Linear Discriminant Analysis Classifier for CHD Prediction
=======================================================================
WHY INCLUDED:
    LDA is a generative linear classifier that models per-class Gaussians
    with a shared covariance matrix. It finds the projection that maximises
    between-class separation relative to within-class scatter. LDA is
    closely related to logistic regression but approaches classification
    from a generative perspective, providing a useful comparison point.
...
def tune_and_evaluate(
    ...
    pd.DataFrame(
        classification_report(y_test, y_pred,
                              ...
                              output_dict=True)
    ).T.to_csv(os.path.join(output_dir, "lda_classification_report.csv"))
...
def _save_figures(model, y_test, y_pred, y_prob, auc, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_lda_roc_curve.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_lda_confusion_matrix.png"), dpi=150)
    plt.close()
