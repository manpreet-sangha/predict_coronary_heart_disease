"""
oc_bagging.py — Bagging Classifier for CHD Prediction
======================================================
WHY INCLUDED:
    Bagging (Bootstrap Aggregating) trains multiple base estimators on
    random bootstrap samples of the data and averages their predictions.
    While Random Forest is a specialised form of bagging (with random
    feature subsets), the generic BaggingClassifier wraps any base
    estimator, providing a more general variance-reduction ensemble.
...
def tune_and_evaluate(
    ...
    pd.DataFrame(
        classification_report(y_test, y_pred,
                              ...
                              output_dict=True)
    ).T.to_csv(os.path.join(output_dir, "bagging_classification_report.csv"))
...
def _save_figures(model, y_test, y_pred, y_prob, auc, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_bagging_roc_curve.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_bagging_confusion_matrix.png"), dpi=150)
    plt.close()
