"""
oc_adaboost.py — AdaBoost Classifier for CHD Prediction
========================================================
WHY INCLUDED:
    AdaBoost is a boosting ensemble that sequentially trains weak learners
    (shallow decision trees), upweighting misclassified samples at each
    iteration. Unlike Random Forest (parallel bagging), AdaBoost reduces
    bias by focusing on hard-to-classify patients, offering a different
    ensemble strategy for comparison.
...
def tune_and_evaluate(
    ...
    pd.DataFrame(
        classification_report(y_test, y_pred,
                              ...
                              output_dict=True)
    ).T.to_csv(os.path.join(output_dir, "adaboost_classification_report.csv"))
...
def _save_figures(model, y_test, y_pred, y_prob, auc, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_adaboost_roc_curve.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_adaboost_confusion_matrix.png"), dpi=150)
    plt.close()
