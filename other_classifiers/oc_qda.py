"""
oc_qda.py — Quadratic Discriminant Analysis Classifier for CHD Prediction
==========================================================================
WHY INCLUDED:
    QDA is a generative classifier that extends LDA by allowing each class
    to have its own covariance matrix. This captures class-specific feature
    correlations, producing quadratic decision boundaries. On small datasets,
    regularisation (reg_param) prevents singular covariance estimates.
...
def tune_and_evaluate(
    ...
    pd.DataFrame(
        classification_report(y_test, y_pred,
                              ...
                              output_dict=True)
    ).T.to_csv(os.path.join(output_dir, "qda_classification_report.csv"))
...
def _save_figures(model, y_test, y_pred, y_prob, auc, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_qda_roc_curve.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_qda_confusion_matrix.png"), dpi=150)
    plt.close()
