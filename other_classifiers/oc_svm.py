"""
oc_svm.py — Support Vector Machine (RBF Kernel) for CHD Prediction
===================================================================
WHY INCLUDED:
    SVM with an RBF kernel finds the maximum-margin decision boundary in a
    high-dimensional feature space without assuming linearity. It is well-
    suited here because the PCA projection showed non-linear class overlap.
    Standardisation is critical for SVM as it is sensitive to feature scale.
...
def tune_and_evaluate(
    ...
    pd.DataFrame(
        classification_report(y_test, y_pred,
                              ...
                              output_dict=True)
    ).T.to_csv(os.path.join(output_dir, "svm_classification_report.csv"))
...
def _save_figures(model, y_test, y_pred, y_prob, auc, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_svm_roc_curve.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "fig_svm_confusion_matrix.png"), dpi=150)
    plt.close()
